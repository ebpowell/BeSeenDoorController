...
synchronizing door controllers with database fobs and access rules based on
the design patterns in documentation/threadsafe_logic.py.

Thread Safety Design:
1. Isolated State Pattern: All sync state (changes_made, batch index, queue) is kept
   strictly as local, call-stack isolated variables. synchronize_access returns
   the total count of verified changes.
2. Unique Instance Per Thread: Each thread instantiates a dedicated AccessSynchronizer
   object on its private memory heap, binding target URLs to default parameters
   to avoid loop closure issues and guarantee zero state-sharing.
3. Isolated DB Connections: Database operations acquire connection contexts
...
class AccessSynchronizer(ControllerScheduler):
    ...
    State variables like changes_made are encapsulated cleanly within method call-stacks.
    ...
    def execute_action(self, controller_url: str, limit_changes=None):
        """
        Executes synchronization using thread-safe call-stack variables and captures total changes.
        """
        log_info(f"Executing synchronization for: {controller_url}")
        total_changes = self.synchronize_access(
            controller_url,
        ...
        log_info(f"Sync complete for {controller_url}. Verified changes: {total_changes}")
    ...
    def synchronize_access(
        ...
        Returns the total verified count of changes made.
        """
    ...
    def start_scheduler_threads(self, controller_urls: List[str], recurrence_interval=None, limit_changes=None) -> List[threading.Thread]:
        """
        Spawns a separate daemon thread for each controller URL using isolated instance pattern.
        """
        ...
        for url in controller_urls:
            # Bind target_url=url to avoid late-binding closure issues in the loop
            def thread_target(target_url=url):
                ...
                # Run the schedule check loop isolated within this instance
...
def start_thread_safe_scheduler(
    ...
    username: str, 
    password: str,
    limit_changes: int = None
) -> List[threading.Thread]:
    """
    Creates a brand-new, isolated AccessSynchronizer object for each thread.
    Guarantees zero state-sharing as defined in documentation/threadsafe_logic.py.
    """
    threads = []
    for url in urls:
        def thread_target(target_url=url):
